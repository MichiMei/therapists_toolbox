...
import Host.Controller.Storage;
...
/*-----------------------------------------------------Controller-----------------------------------------------------*/
    ...
    public HostMainPane (HostController controller) {
        ...
        workSheetsPanel.add(new JLabel(new ImageIcon(this.getClass().getResource("/Resources/Icons/loading.gif"))));
    ...
    public void connectionStatusChanged(HostConnector.Status status, String msg) {
        ...
        switch (status) {
            case Online -> {
                ...
                assert (own != null);
                addressTextField.setText(own.getIP());
            }
            case Offline -> {
                onlineToggleButton.setEnabled(true);
